...
var (
	...
	ifRepoMapBlockRegex = regexp.MustCompile(`(?s)\{\{#if_repo_map\}\}(.*?)\{\{/if_repo_map\}\}`)
	ifCallersBlockRegex = regexp.MustCompile(`(?s)\{\{#if_callers_context\}\}(.*?)\{\{/if_callers_context\}\}`)
	ifCalleeBlockRegex  = regexp.MustCompile(`(?s)\{\{#if_callee_context\}\}(.*?)\{\{/if_callee_context\}\}`)
...
func (s *AIService) Review(ctx context.Context, req *ReviewRequest) (*ReviewResult, error) {
	...
	prompt = s.processRepoMapBlock(prompt, req.RepoMap)
	prompt = s.processCallersContextBlock(prompt, req.CallersContext)
	prompt = s.processCalleeContextBlock(prompt, req.CalleeContext)

	logger.Infof("[AI] Prompt length: %d chars, Diffs length: %d chars, Commits length: %d chars, FileContext length: %d chars, RepoMap length: %d chars",
		len(prompt), len(req.Diffs), len(req.Commits), len(req.FileContext), len(req.RepoMap))
...
func (s *AIService) callOllama(ctx context.Context, llmConfig *models.LLMConfig, prompt string) (*ReviewResult, error) {
	...
	// Strip <think> tags from content
	result = thinkBlockRegex.ReplaceAllString(result, "")
...
func (s *AIService) processFileContextBlock(prompt string, fileContext string) string {
	if strings.TrimSpace(fileContext) != "" {
		// If {{#if_file_context}} exists, replace block with content
		prompt = ifBlockRegex.ReplaceAllString(prompt, "$1")
		// If only {{file_context}} exists (no block), replace it directly
...
func (s *AIService) processRepoMapBlock(prompt string, repoMap string) string {
	if strings.TrimSpace(repoMap) != "" {
		// If {{#if_repo_map}} exists, replace block with content
		prompt = ifRepoMapBlockRegex.ReplaceAllString(prompt, "$1")
		// If only {{repo_map}} exists (no block), replace it directly
		prompt = strings.ReplaceAll(prompt, "{{repo_map}}", repoMap)
	} else {
		// Remove block
		prompt = ifRepoMapBlockRegex.ReplaceAllString(prompt, "")
		// Remove tag
		prompt = strings.ReplaceAll(prompt, "{{repo_map}}", "")
	}

	return prompt
}

func (s *AIService) processCallersContextBlock(prompt, callersContext string) string {
	if strings.TrimSpace(callersContext) != "" {
		// If {{#if_callers_context}} exists, replace block with content
		prompt = ifCallersBlockRegex.ReplaceAllString(prompt, "$1")
		// If only {{callers_context}} exists (no block), replace it directly
		prompt = strings.ReplaceAll(prompt, "{{callers_context}}", callersContext)
	} else {
		// Remove block
		prompt = ifCallersBlockRegex.ReplaceAllString(prompt, "")
		// Remove tag
		prompt = strings.ReplaceAll(prompt, "{{callers_context}}", "")
	}
	return prompt
}

func (s *AIService) processCalleeContextBlock(prompt, calleeContext string) string {
	if strings.TrimSpace(calleeContext) != "" {
		prompt = ifCalleeBlockRegex.ReplaceAllString(prompt, "$1")
		prompt = strings.ReplaceAll(prompt, "{{callee_context}}", calleeContext)
	} else {
		prompt = ifCalleeBlockRegex.ReplaceAllString(prompt, "")
		prompt = strings.ReplaceAll(prompt, "{{callee_context}}", "")
	}
	return prompt
}
...
func extractScore(content string) float64 {
	// Strip <think> blocks first to avoid matching intermediate scores from AI reasoning
	content = thinkBlockRegex.ReplaceAllString(content, "")

	// Strip markdown formatting (bold, italic, code) that may wrap around scores
	// e.g. "总分: **85**分" → "总分: 85分"
	cleaned := markdownFmtRegex.ReplaceAllString(content, "")
...
func (s *AIService) ReviewChunked(ctx context.Context, req *ReviewRequest) (*ReviewResult, error) {
	...
	// Combine final prompts from all batches to return a complete prompt history
	var combinedPrompts strings.Builder
	for _, br := range batchResults {
		combinedPrompts.WriteString(fmt.Sprintf("\n\n=== Batch %d (Files: %s) ===\n\n", br.BatchIndex+1, strings.Join(br.Files, ", ")))
		combinedPrompts.WriteString(br.FinalPrompt)
	}
	...
	return &ReviewResult{
		...
		FinalPrompt: combinedPrompts.String(),
	}, nil
}
