...
func (s *DashboardService) GetStats(req *DashboardStatsRequest) (*DashboardResponse, error) {
	...
	s.db.Model(&models.ReviewLog{}).
		Where("created_at BETWEEN ? AND ? AND score IS NOT NULL AND is_manual = ?", startDate, endDate, false).
		Select("COALESCE(AVG(score), 0)").
		Scan(&stats.AverageScore)

	var projectStats []ProjectStats
	s.db.Model(&models.ReviewLog{}).
		Select("project_id, COUNT(*) as commit_count, COALESCE(AVG(CASE WHEN is_manual = ? THEN score END), 0) as avg_score, COALESCE(SUM(additions), 0) as additions, COALESCE(SUM(deletions), 0) as deletions", false).
		Where("created_at BETWEEN ? AND ?", startDate, endDate).
		Group("project_id").
	...
	s.db.Model(&models.ReviewLog{}).
		Select("author, COUNT(*) as commit_count, COALESCE(AVG(CASE WHEN is_manual = ? THEN score END), 0) as avg_score, COALESCE(SUM(additions), 0) as additions, COALESCE(SUM(deletions), 0) as deletions", false).
		Where("created_at BETWEEN ? AND ?", startDate, endDate).
		Group("author").
		Order("commit_count DESC").
		Limit(authorLimit).
