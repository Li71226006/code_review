...
func ParseDiffToFiles(diff string) []FileDiff {
	...
	// Split by "diff --git" blocks first (standard Git diff format)
	diffPattern := regexp.MustCompile(`(?m)^diff --git a/(.+?) b/(.+?)$`)
	indices := diffPattern.FindAllStringIndex(diff, -1)

	if len(indices) == 0 {
		// Fallback for GitHub API formats that might not have "diff --git"
		// They usually start with "--- a/path\n+++ b/path"
		fallbackPattern := regexp.MustCompile(`(?m)^--- a/(.+?)\n\+\+\+ b/(.+?)$`)
		fallbackIndices := fallbackPattern.FindAllStringIndex(diff, -1)
		...
		for i, idx := range fallbackIndices {
			start := idx[0]
			end := len(diff)
			if i+1 < len(fallbackIndices) {
				end = fallbackIndices[i+1][0]
			}

			block := diff[start:end]
			matches := fallbackPattern.FindStringSubmatch(block)
