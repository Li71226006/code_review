...
// GetWithDefault returns a config value or a default if not found
func (s *SystemConfigService) GetWithDefault(key string, defaultValue string) string {
...
type FileContextConfigResponse struct {
	ReviewMode  string `json:"review_mode"`   // v0 (pure diff), v1 (diff+10lines), v2 (AST repomap+callers)
...
func (s *SystemConfigService) GetFileContextConfig() *FileContextConfigResponse {
	...
	// Legacy migration check: if the new key doesn't exist, we migrate from old keys to determine the best default
	reviewMode := s.GetWithDefault("file_context_review_mode", "")
	if reviewMode == "" {
		legacyEnabled := s.GetWithDefault("file_context_enabled", "false") == "true"
		legacyRepoMap := s.GetWithDefault("file_context_repo_map_enabled", "true") == "true"
		...
		s.Set("file_context_review_mode", reviewMode)
	}
...
func (s *SystemConfigService) UpdateFileContextConfig(req *UpdateFileContextConfigRequest) error {
	if req.ReviewMode != nil {
		if err := s.Set("file_context_review_mode", *req.ReviewMode); err != nil {
			return err
		}
	}
