...
func (s *Service) processBitbucketPush(ctx context.Context, project *models.Project, event *BitbucketPushEvent) error {
	...
	for _, change := range event.Push.Changes {
		...
		for _, c := range change.Commits {
			commits = append(commits, fmt.Sprintf("%.8s: %s", c.Hash, c.Message))
		}
		...
		if !isNullSHA(beforeSHA) && beforeSHA != "" {
			...
			if err != nil {
				...
				diff = compareDiff
				logger.Infof("[Webhook] Got Bitbucket compare diff (before=%.8s, after=%.8s), length: %d bytes",
					beforeSHA, commitSHA, len(diff))
			}
		}
		...
		logger.Infof("[Webhook] Bitbucket push review task enqueued for project %d, commit %.8s", project.ID, commitSHA)
	}
...
func (s *Service) getBitbucketCompareDiff(project *models.Project, from, to string) (string, error) {
	...
	logger.Infof("[Webhook] Fetching Bitbucket compare diff: %.8s...%.8s", from, to)
