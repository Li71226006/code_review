...
func (s *Service) processGitHubPush(ctx context.Context, project *models.Project, event *GitHubPushEvent) error {
	...
	if !isNullSHA(event.Before) && event.Before != "" {
		...
		if err != nil {
			...
			diff = compareDiff
			logger.Infof("[Webhook] Got GitHub compare diff (before=%.8s, after=%.8s), length: %d bytes",
				event.Before, event.After, len(diff))
		}
	}
	...
	logger.Infof("[Webhook] GitHub push review task enqueued for project %d, commit %.8s", project.ID, event.After)
	return nil
}
...
func (s *Service) getGitHubCompareDiff(project *models.Project, before, after string) (string, error) {
	...
	logger.Infof("[Webhook] Fetching GitHub compare diff: %.8s...%.8s", before, after)
	return s.fetchGitHubDiff(apiURL, project.AccessToken)
}
