...
func (s *SystemLogService) GetRetentionDays() int {
	retentionDaysStr := s.configService.GetWithDefault("log_retention_days", "30")
	days, err := strconv.Atoi(retentionDaysStr)
	if err != nil {
		logger.Errorf("[SystemLog] Invalid log retention days config: %v, value: %s", err, retentionDaysStr)
		// Auto fix invalid config
		s.configService.Set("log_retention_days", "30")
		return 30
	}
	return days
}
