...
The following is a structural map of the related files in the repository. Use this to understand the context of definitions and references across files.
{{repo_map}}

{{/if_repo_map}}
{{#if_callers_context}}
**Cross-File Callers Context (V3 Analysis)**:
The following functions across the repository call the code you just modified. Please review to ensure the modifications don't break these callers.
{{callers_context}}
...
{{#if_repo_map}}
**代码仓库结构映射 (Repo Map)**：
以下是相关文件的结构映射图。请利用它来理解跨文件的定义和引用关系，检查是否存在调用错误。
{{repo_map}}

{{/if_repo_map}}
{{#if_callers_context}}
**跨文件调用者上下文 (V3 静态分析)**：
以下是代码仓库中调用了本次被修改函数的相关代码。请结合这些调用方代码进行审查，确保本次修改没有破坏其他文件的调用逻辑（如参数数量变更、返回类型变更或异常处理变更）。
{{callers_context}}
