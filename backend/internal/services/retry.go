...
func NewRetryService(db *gorm.DB, aiCfg *config.OpenAIConfig) *RetryService {
	...
	return &RetryService{
		...
		fileContextService:  fileContextService,
		repoMapService:      NewRepoMapService(fileContextService),
	}
}
...
func (s *RetryService) retryReview(review *models.ReviewLog) {
	...
	if reviewMode == "v1" || reviewMode == "v2" {
		fileContext, rawFilesContext, _ = s.fileContextService.BuildFileContext(&project, diff, review.CommitHash)
		editedFuncs, calledFuncs = s.fileContextService.GetModifiedFunctionNames(&project, diff, review.CommitHash)
	}
	...
	if reviewMode == "v2" {
		...
		repoMap, globalFilesMap, _ = s.repoMapService.GetRepoMap(context.Background(), &project, review.CommitHash)

		// Fallback to diff files only if global map fails or is empty
		if repoMap == "" {
			filesMap, _ := s.fileContextService.GetFilesContentMap(&project, diff, review.CommitHash)
			if len(filesMap) > 0 {
		...
		if repoMap != "" {
			logger.Infof("[Retry] Generated Repo Map: %d chars", len(repoMap))
		}
		if callersContext != "" {
			logger.Infof("[Retry] Generated Callers Context: %d chars", len(callersContext))
		}
		if calleeContext != "" {
			logger.Infof("[Retry] Generated Callee Context: %d chars", len(calleeContext))
		}
	}

	result, err := s.aiService.ReviewChunked(context.Background(), &ReviewRequest{
	...
	if err != nil {
		...
		review.FinalPrompt = result.FinalPrompt
		review.RawFilesContext = rawFilesContext
		review.ErrorMessage = ""
