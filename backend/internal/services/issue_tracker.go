	...
	for _, tracker := range trackers {
		if *reviewLog.Score < tracker.ScoreThreshold {
			req := CreateIssueRequest{
				Title:       fmt.Sprintf("[CodeSentry] Low Score Review: %.8s (%.0f/100)", reviewLog.CommitHash, *reviewLog.Score),
			...
			if err := s.createIssue(&tracker, &req); err != nil {
				...
				logger.Infof("[IssueTracker] Created issue in %s for commit %.8s", tracker.Name, reviewLog.CommitHash)
			}
		}
	}
}
