...
sequenceDiagram
    ...
    participant Webhook as CodeSentry Webhook 接收层
    participant Queue as Redis 异步队列 / Sync 引擎
    participant Parser as CodeSentry 智能解析流水线
    participant LLM as 大语言模型 (AI)

    Git->>Webhook: 1. 触发 Push / MR 事件 (包含 Commit SHA)
    Webhook->>Queue: 2. 生成审查任务并调度
    Queue->>Parser: 3. 开始处理审查任务
    
    rect rgb(240, 248, 255)
        Note over Parser, Git: 源码获取与前置过滤
        Parser->>Git: 4. 通过 API 拉取该 Commit 的 Diff 补丁
        Git-->>Parser: 返回 Diff 补丁
        Parser->>Parser: 5. 过滤跳过无关文件 (如 lock, 静态资源)
        Parser->>Parser: 6. 按代码修改量排序，提取 Top N 个核心文件
        Parser->>Git: 7. 通过 API 拉取这 N 个文件的完整源代码
        Git-->>Parser: 返回完整源代码
        Parser->>Parser: 8. 丢弃体积超过 Max File Size 的超大文件
    end

    rect rgb(255, 245, 238)
        Note over Parser: 双引擎上下文解析
        alt V1 模式 (轻量)
            Parser->>Parser: 9a. 纯文本扩展: 提取 Diff 上下 10 行并智能合并
        else V2 模式 (深度)
            Parser->>Parser: 9b. AST 解析: 提取完整函数/类块
            Parser->>Parser: 9c. 向上追溯 (Callers): 全网查找谁调用了被改函数
            Parser->>Parser: 9d. 向下校验 (Callee): 提取被改代码调用的底层函数定义
            Parser->>Parser: 9e. 兜底捕获 (Orphan): 提取全局变量/import等孤儿代码
        end
    end

    Parser->>Parser: 10. 组装最终 Prompt (注入 file, callers, callee 变量)

    rect rgb(245, 255, 250)
        Note over Parser, LLM: 智能分批与 AI 审查
        alt Token 未超标
            Parser->>LLM: 11a. 单次发送全量 Prompt
            LLM-->>Parser: 返回审查结果
        else Token 超标 (Chunking)
            Parser->>Parser: 11b. 将 Prompt 按文件粒度拆分为多个 Batch
            par 并发请求
                Parser->>LLM: 发送 Batch 1
                Parser->>LLM: 发送 Batch 2
            end
            LLM-->>Parser: 返回各 Batch 审查结果
            Parser->>Parser: 11c. 聚合结果并加权计算最终得分
        end
    end

    Parser->>Git: 12. 回写审查报告 (Commit Comment / PR Review)
    Parser->>Webhook: 13. (可选) 拦截不合格的提交 (Sync 模式)
    Parser->>Git: 发送 IM 消息通知 (钉钉/飞书等)
```
