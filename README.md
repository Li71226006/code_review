...
graph TD
    subgraph 触发层
        A1[GitLab/GitHub] -->|异步 Webhook: Push / MR| B1(Webhook 接收层)
        A2[Git Server Hooks] -->|同步请求: Pre-receive| B2(Sync API 接收层)
    end

    subgraph 调度层
        B1 -->|放入 Redis 异步队列| C1[Task Queue Worker]
        B2 -->|直接进入处理逻辑| C2[Sync Review Engine]
    end

    subgraph 智能解析流水线
        C1 & C2 --> D{前置过滤}
        D -->|跳过超大文件| E[按代码修改量提取 Top N 文件]
        E --> F{选择解析引擎}
        F -->|V1 模式| G[纯文本上下文扩展与合并]
        F -->|V2 模式| H[AST 语法树解析与跨文件依赖追溯]
        G & H --> I[注入 Prompt 模板变量]
    end

    subgraph 大模型处理层
        I --> J{检查 Token 长度}
        J -->|未超标| K[直接请求大模型]
        J -->|超标| L[按文件粒度分批 Chunking 并发请求]
        K & L --> M[聚合审查结果与打分]
    end

    subgraph 反馈层
        M -->|异步 Webhook 链路| N[回写 Git 平台 Comment & IM 消息通知]
        M -->|同步 API 链路| O[立即返回 JSON 结果以拦截不合格代码]
    end
```
