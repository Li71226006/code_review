        ...
        {selectedLog && (
          <>
            ...
            {selectedLog.final_prompt && (
              <Card title="最终发给 AI 的完整上下文 (Final Prompt)" size="small" style={{ marginTop: 16 }}>
                <pre style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-all', backgroundColor: '#f5f5f5', padding: '12px', borderRadius: '4px', maxHeight: '400px', overflowY: 'auto' }}>
                  {selectedLog.final_prompt}
                </pre>
              </Card>
            )}

            {selectedLog.raw_files_context && (
              <Card title="后端抓取的原始文件内容 (Raw Files Debug)" size="small" style={{ marginTop: 16 }}>
                <Paragraph type="secondary" style={{ marginBottom: 8 }}>
                  以下内容为后端通过 API 拉取到的文件完整源码，用于给 Tree-sitter 切割上下文或构建 RepoMap。如果为空或抓取错误，说明平台 API Token 权限不足或文件超过了大小限制。
                </Paragraph>
                <pre style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-all', backgroundColor: '#e6f7ff', padding: '12px', borderRadius: '4px', maxHeight: '400px', overflowY: 'auto', border: '1px solid #91d5ff' }}>
                  {selectedLog.raw_files_context}
                </pre>
              </Card>
            )}
