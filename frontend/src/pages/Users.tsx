    ...
    <>
      <Card>
        <Space style={{ marginBottom: 16 }} wrap>
          ...
          <Button type="primary" onClick={() => setIsCreateModalVisible(true)}>{t('users.createUser')}</Button>
        </Space>
      ...
      <Modal title={t('users.createUser')} open={isCreateModalVisible} onOk={handleCreate} onCancel={() => setIsCreateModalVisible(false)} confirmLoading={createUser.isPending} width={480}>
        <Form form={createForm} layout="vertical">
          <Form.Item name="username" label={t('users.username')} rules={[{ required: true }, { min: 3, max: 50 }]}>
            <Input />
          </Form.Item>
          <Form.Item name="password" label={t('users.password')} rules={[{ required: true }, { min: 6 }]}>
            <Input.Password />
          </Form.Item>
          <Form.Item name="nickname" label={t('users.nickname')} rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item name="email" label={t('users.email')} rules={[{ type: 'email' }]}>
            <Input />
          </Form.Item>
          <Form.Item name="role" label={t('users.role')} rules={[{ required: true }]} initialValue="user">
