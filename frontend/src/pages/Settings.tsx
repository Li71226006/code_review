...
const Settings: React.FC = () => {
  ...
  useEffect(() => {
    if (fileContextConfig) {
      fileContextForm.setFieldsValue({
        review_mode: fileContextConfig.review_mode || 'v0',
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      <Card title="上下文与跨文件审查配置 (V3架构)" extra={<Button type="primary" icon={<SaveOutlined />} loading={updateFileContext.isPending} onClick={handleFileContextSave}>{t('common.save')}</Button>}>
        <Form form={fileContextForm} layout="vertical" style={{ maxWidth: 600 }}>
          <Form.Item name="review_mode" label="审查模式选择">
            <Radio.Group style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <Radio value="v0">
                <b>V0 基础版 (纯净 Diff)</b>
                <div style={{ color: '#888', fontSize: '12px', marginTop: '4px', whiteSpace: 'normal' }}>
                  仅提供修改前后的基础代码差异，自动过滤掉纯空行等无用修改。适合极其简单的拼写修改。
                </div>
              </Radio>
              <Radio value="v1">
                <b>V1 进阶版 (Diff + 10行上下文)</b>
                <div style={{ color: '#888', fontSize: '12px', marginTop: '4px', whiteSpace: 'normal' }}>
                  在纯净 Diff 的基础上，智能提取上下各 10 行代码作为局部上下文，并自动合并相邻改动，解决 80% 的单文件审查需求。（推荐）
                </div>
              </Radio>
              <Radio value="v2">
                <b>V2 终极版 (AST 精准上下文 + 全局静态分析)</b>
                <div style={{ color: '#888', fontSize: '12px', marginTop: '4px', whiteSpace: 'normal' }}>
                  抛弃粗暴的行数提取，利用 Tree-sitter 引擎精准提取 Diff 所在的完整函数/类代码块。同时构建全局 RepoMap 进行静态分析，将调用了被修改函数的上游代码一并发送给 AI，完美解决跨文件影响盲区。（强烈推荐）
                </div>
              </Radio>
            </Radio.Group>
          </Form.Item>
          <Row gutter={16} style={{ marginTop: '16px' }}>
            <Col xs={24} sm={12}><Form.Item name="max_file_size" label="最大允许解析的文件大小" extra="超过此大小的文件将被跳过，防止Token爆表"><InputNumber min={1024} max={10485760} step={1024} style={{ width: '100%' }} addonAfter="bytes" /></Form.Item></Col>
            <Col xs={24} sm={12}><Form.Item name="max_files" label="单次最大遍历文件数" extra="为了保证性能，单次审查最多提取这么多文件的语法树"><InputNumber min={1} max={100} style={{ width: '100%' }} /></Form.Item></Col>
          </Row>
        </Form>
      </Card>
